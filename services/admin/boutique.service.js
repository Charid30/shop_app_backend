...
// This file contains the service functions for managing boutiques in the admin database.
...
async function createBoutique(boutiqueData) {
    ...
    try {
        ...
        const [existing] = await admindb.query(checkQuery, [boutiqueData.nom_boutique]);
        ...
        const [result] = await admindb.query(insertQuery, [boutiqueData.nom_boutique, boutiqueData.adresse_boutique, boutiqueData.telephone_boutique]);
...
async function updateBoutique(id, boutiqueData) {
    ...
    try {
        const [result] = await admindb.query(query, [boutiqueData.nom_boutique, boutiqueData.adresse_boutique, boutiqueData.telephone_boutique, id]);
