...
async function createArticle(articleData) {
    ...
    try {
        ...
        const [existing] = await pool.query(checkQuery, [articleData.nom_articles]);
        if (existing.length > 0) {
            console.log("[createArticle] Article already exists:", existing);
            return { error: "Cet article existe déjà" };
        }
        // Insert the new article
        const [result] = await pool.query(insertQuery, [articleData.nom_articles, articleData.description_articles, articleData.prix_articles, articleData.stock_articles]);
...
async function updateArticle(id, articleData) {
    ...
    try {
        const [result] = await pool.query(query, [articleData.nom_articles, articleData.description_articles, articleData.prix_articles, articleData.stock_articles, id]);
